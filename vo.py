...
def update_estimated_time_label(time_sec):  # Функция обновления метки с оценочным временем конвертации
    ...
    else:  # Если время составляет 3600 секунд и более (один час или больше)
...
def convert_file(file_path, preset, crf, should_delete_source):  # Функция для конвертации одного видеофайла с помощью ffmpeg и кодека libsvtav1
    ...
    ffprobe_pid = None  # Инициализация переменной для хранения PID процесса ffprobe
    ...
    try:
        ...
        # Если процесс ffprobe все еще активен, завершаем его перед переходом к следующей конвертации
        if ffprobe_process is not None and ffprobe_process.poll() is None:  # Если процесс ffprobe существует и еще не завершился
            try:  # Начинаем попытку корректного завершения процесса ffprobe
                logger.info(f"Попытка завершить процесс ffprobe с PID {ffprobe_pid}")  # Логирование попытки завершения процесса с указанным PID
                ffprobe_process.terminate()  # Отправляем сигнал завершения процессу ffprobe
                ffprobe_process.wait(timeout=5)  # Ожидаем завершения процесса в течение 5 секунд
                remaining = subprocess.check_output(f'tasklist /FI "PID eq {ffprobe_pid}"', shell=True, universal_newlines=True)  # Получаем список процессов для проверки, активен ли процесс с данным PID
                if str(ffprobe_pid) in remaining:  # Если PID процесса найден в списке активных процессов
                    logger.info(f"Процесс с PID {ffprobe_pid} все еще запущен, пробуем taskkill")  # Логирование, что процесс не завершился, и переходим к принудительному завершению
                    subprocess.run(f'taskkill /F /T /PID {ffprobe_pid}', shell=True)  # Принудительное завершение процесса с помощью команды taskkill
                    time.sleep(1)  # Ждем 1 секунду для стабилизации состояния системы после taskkill
                    remaining = subprocess.check_output(f'tasklist /FI "PID eq {ffprobe_pid}"', shell=True, universal_newlines=True)  # Снова получаем список процессов для проверки завершения процесса
                    if str(ffprobe_pid) in remaining:  # Если процесс все еще обнаружен в списке
                        logger.error(f"Не удалось завершить процесс ffprobe с PID {ffprobe_pid}")  # Логирование ошибки, так как принудительное завершение не сработало
                    else:
                        logger.info(f"Процесс ffprobe с PID {ffprobe_pid} успешно завершен после taskkill")  # Логирование успешного принудительного завершения процесса
                else:
                    logger.info(f"Процесс ffprobe с PID {ffprobe_pid} успешно завершен")  # Логирование успешного завершения процесса без необходимости принудительного завершения
            except Exception as e:  # Обработка исключений, возникших при завершении процесса
                logger.error(f"Ошибка при завершении процесса ffprobe: {e}")  # Логирование ошибки с описанием исключения
...
description_label = tk.Label(root, text=desc_text, font=(None, 7), wraplength=600, justify="left", anchor="w", bg="#1e1e1e", fg="#d4d4d4")
